...
change_string = '4 models ensembled. 4 fixed, 1 train, 2 patience, with new cleaning AND 5% validation.\n'
...
class LSTMNet(nn.Module):
    def __init__(self, embedding_matrix):
        ...
        self.gru_1 = nn.LSTM(embed_size, hidden_size, bidirectional=True, batch_first=True)
        self.gru_2 = nn.LSTM(hidden_size*2, hidden_size, bidirectional=True, batch_first=True)
...
class GRULSTMNet(nn.Module):
    def __init__(self, embedding_matrix):
        super(GRULSTMNet, self).__init__()
...
model_list = [GRUNet, LSTMNet, GRULSTMNet, LSTMGRUNet]

train_idx, valid_idx = list(StratifiedKFold(n_splits=20, shuffle=True, random_state=seed).split(X_train, train_labels))[0]
...
oof_preds_fold = np.zeros((valid_x.size(0)))
...
for i in range(0, len(model_list)):
    ...
    final_preds += test_preds_fold/len(model_list)
    oof_preds_fold += valid_preds_fold/len(model_list)
    
    del embedding_matrix, model
    gc.collect()

optimal_threshold = threshold_search(valid_y.cpu().numpy(), oof_preds_fold)
